...
describe('installer', () => {
    ...
    test('is expected to throw if the platform is not supported', async () => {
        ...
        await expect(install('13.2.3', testInstallPath)).rejects.toThrow('There is not an embedded-postgres-binaries available for');
        spy.mockRestore();
    });
    ...
    test('is expected to throw if the platform and arch combination is not supported', async () => {
        ...
        const archSpy = jest.spyOn(os, 'arch').mockImplementation(() => 'powerpc64le');
        await expect(install('13.2.3', testInstallPath)).rejects.toThrow('There is not an embedded-postgres-binaries avaliable for the combination of');
    ...
    test('is expected to throw if an installation already exists', async () => {
        await expect(install('13.2.3', testInstallPath)).rejects.toThrow('Already installed');
    });
